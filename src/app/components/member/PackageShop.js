...
export default function PackageShop({ packages, dashboardData, userData }) {
  const [isBuying, setBuying] = useState(false);
  const [cart, setCart] = useState([]);

  function AddToCart(packageItem, status) {
    const cleanPackage = {
      ...packageItem,
    ...
    setCart((prev) => {
      const existing = prev.find((item) => item.id === cleanPackage.id);

      if (status !== "del") {
        if (existing) {
          return prev.map((item) =>
            item.id === cleanPackage.id
              ? { ...item, quantity: item.quantity + 1 }
              : item
          );
        }
        return [...prev, cleanPackage];
      }
  ...
  return (
    <div>
      ...
      <div className="mb-5 p-3 text-blue-600 border border-blue-400 rounded-2xl flex flex-col gap-2 sm:flex-row sm:justify-between sm:items-center">
        <span>Explore and shop high-quality premium packages!</span>
      ...
      {/* PACKAGE GRID (merged from PackagesMember) */}
      {packages.length === 0 ? (
        <p className="text-center text-gray-500">No packages loaded</p>
      ) : (
