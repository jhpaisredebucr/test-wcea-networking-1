...
export default function DashboardMember({dashboardData, userData}) {
    ...
    return (
        <div className="grid grid-cols-4 auto-rows-[130px] gap-5 w-full">
            <Card title="Available Balance" src="/icons/money.svg" color="bg-blue-200" value={`₱${dashboardData?.userBalance}`} valueSize="text-4xl" clas info="" rowSpan="row-span-2" colSpan="col-span-2">
                <button className="p-2 bg-(--primary) hover:bg-(--primary)/80 hover:shadow-md cursor-pointer rounded-lg">
                    <p className="text-white">
                        Withdraw
                    </p>
                </button>
            </Card>
            <Card title="Total Commissions" src="/icons/money.svg" color="bg-orange-200" value={dashboardData?.totalReferredMembers} info="" colSpan="col-span-2"/>
            ...
            <Card title="Referral Link" value="" valueSize="lg" bold="" info=" " colSpan="col-span-4">
                <div className="flex justify-between items-center border-2 border-gray-200 border-dotted p-2 rounded-xl">
                    ...
                    <button
                        onClick={handleCopy}
                        className="flex cursor-pointer hover:bg-(--primary)/80 hover:shadow-md duration-150 gap-x-2 bg-(--primary) p-3 rounded-xl text-white">
